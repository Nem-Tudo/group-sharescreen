...
const NOTIFY_LABELS: Record<GroupNotifyLevel, string> = {
  get all() { return translate("groups.groupSidebar.allMessages"); },
  get mentions() { return translate("groups.groupSidebar.mentionsAndRepliesOnly"); },
  get none() { return translate("groups.groupSidebar.nothing"); },
};
...
export function GroupRoomsPanel({
  ...
  function confirmDeleteCategory(category: GroupCategory) {
    ...
    void openPopup("confirm", {
      data: {
        title: t("groups.groupSidebar.deleteTheNameCategory", { name: category.name }),
        message: t("groups.groupSidebar.itsRoomsAreNotDeletedThey"),
        cancelLabel: t("common.cancel"),
        confirmLabel: t("common.deleteCategory"),
  ...
  const editButton = (channelId: string) => (
    <span
      role="button"
      tabIndex={0}
      aria-label={t("groups.groupSidebar.roomSettings")}
      title={t("groups.groupSidebar.roomSettings")}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        onNavigate?.();
  ...
  function renderVoiceRoom(channel: GroupChannel) {
    ...
    const headerContent = (
      <>
        {locked ? (
          <MdLock className="h-4 w-4 shrink-0 opacity-60" aria-label={t("groups.groupSidebar.locked")} />
        ) : (
        ...
        <span className="flex min-w-0 flex-1 items-center gap-1">
          ...
          {music && (
            <MdMusicNote
              ...
              title={music.playing ? t("groups.groupSidebar.musicPlaying") : t("groups.groupSidebar.musicPaused")}
              aria-label={music.playing ? t("groups.groupSidebar.musicPlaying") : t("groups.groupSidebar.musicPaused")}
            />
          )}
        </span>
        {connected ? (
          <span className="shrink-0 rounded-full bg-emerald-600 px-2 py-0.5 text-[11px] font-medium text-white">
            {t("groups.groupSidebar.youAreHere")}
          </span>
        ) : people.length === 0 ? (
    ...
    >
      <p className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
        {creating.kind === "category"
          ? t("groups.groupSidebar.newCategory")
          : creating.kind === "voice"
            ? t("groups.groupSidebar.newVoiceRoom")
            : t("groups.groupSidebar.newTextRoom")}
      </p>
      <div className="flex gap-1.5">
        <input
          ...
          maxLength={32}
          placeholder={creating.kind === "category" ? t("groups.groupSidebar.exGames") : creating.kind === "voice" ? t("groups.groupSidebar.exGameNight") : t("groups.groupSidebar.exMemes")}
        ...
        {isManager && renaming?.id !== category.id && (
          <>
            <Popover
              ...
              tooltip={t("groups.groupSidebar.editCategory")}
              content={
            ...
            >
              <button
                ...
                aria-label={t("groups.groupSidebar.editCategory")}
            ...
            <Popover
              ...
              placement="bottom-end"
              tooltip={t("groups.groupSidebar.createARoomInThisCategory")}
              content={
                <div className="flex w-48 flex-col gap-0.5 rounded-xl border border-zinc-200 bg-white p-1.5 shadow-xl dark:border-zinc-800 dark:bg-zinc-950">
            ...
            >
              <button
                ...
                aria-label={t("groups.groupSidebar.createARoomInName", { name: category.name })}
    ...
    return (
      ...
      >
        ...
        {isManager && empty && !isCollapsed && (category || drag?.type === "channel") && (
          ...
          >
            {category ? t("groups.groupSidebar.dragRoomsHere") : t("groups.groupSidebar.dropHereToTakeItOut")}
          </p>
        )}
      </section>
    );
  ...
  const content = (
    <div className="flex flex-col gap-3" onContextMenu={panelMenu}>
      ...
      {!hasCategories && channels.length === 0 && (
        <p className="px-1 text-xs text-zinc-400 dark:text-zinc-500">{t("groups.groupSidebar.noRoom")}</p>
      )}
    </div>
  );
...
export function VoiceControls({ className = "" }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {controls && (
        <Tooltip content={controls.isMicOn ? t("groups.groupSidebar.turnMicrophoneOff") : t("groups.groupSidebar.turnMicrophoneOn")}>
          <button
            type="button"
            onClick={controls.toggleMic}
            aria-label={controls.isMicOn ? t("groups.groupSidebar.turnMicrophoneOff") : t("groups.groupSidebar.turnMicrophoneOn")}
      ...
      <Tooltip content={t("common.leaveTheCall")}>
        <button
          type="button"
          onClick={() => {
            playHangUpSound();
...
export function VoiceCallLink() {
  ...
  return (
    <Tooltip
      content={call.group ? t("groups.groupSidebar.backToTheCallGroupname", { groupName: call.group.groupName }) : t("common.backToTheCall")}
      placement="bottom"
    >
      <GroupLink
        href={callPathFor(call)}
        aria-label={t("groups.groupSidebar.backToTheCallInName", { name })}
