      ...
      >
        ...
        {allowed && status !== "finishing" && (
          <div className="flex flex-col gap-2 border-t border-zinc-100 px-5 py-3 dark:border-zinc-800">
            {!busy && othersTranscribing.length > 0 && (
              // The room shares one transcript (lib/liveTranscriptShare):
              // starting a second one costs nothing extra, so this informs
              // rather than asks.
              <p className="text-xs font-medium text-violet-700 dark:text-violet-300">
                {t("callTranscript.alreadyShared", { names: othersTranscribing.join(", ") })}
              </p>
            )}
