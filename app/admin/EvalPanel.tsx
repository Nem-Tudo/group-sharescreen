...
export function EvalPanel() {
  ...
  async function run() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("admin.evalPanel.couldNotSend"));
    } finally {
      setRunning(false);
    }
  }
  ...
  return (
    <div className="rounded-xl border border-red-300 bg-red-50/40 p-4 dark:border-red-900/60 dark:bg-red-950/20">
      <h2 className="text-sm font-semibold text-red-700 dark:text-red-400">
        {t("admin.evalPanel.runCodeOnTheClients")}
      </h2>
      <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">
        {t("admin.evalPanel.sendsASnippetOfJavascriptTo")}
      </p>

      <label className="mt-3 block text-xs font-medium text-zinc-600 dark:text-zinc-400">
        {t("admin.evalPanel.code")}
      </label>
      <textarea
        value={code}
        ...
        rows={5}
        spellCheck={false}
        placeholder={t("admin.evalPanel.exConsoleLogHiLocationPathname")}
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
          {t("admin.evalPanel.targetFilters")}
        </span>
        {rows.length > 1 && (
          <div className="inline-flex overflow-hidden rounded-md border border-zinc-300 text-xs dark:border-zinc-700">
            {(["and", "or"] as const).map((mode) => (
              ...
              >
                {mode === "and" ? t("admin.evalPanel.andAll") : t("admin.evalPanel.orAny")}
              </button>
            ))}
          </div>
        )}
      ...
      {noFilter ? (
        <p className="mt-2 rounded-md bg-amber-100 px-3 py-2 text-xs text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
          {t("admin.evalPanel.noFiltersTheCodeRunsOn")} <strong>{t("common.allWord")}</strong> {t("admin.evalPanel.connectedClientsSuffix")}
        </p>
      ) : (
              ...
              <div key={row.id} className="flex flex-wrap items-center gap-1.5">
                {index > 0 && (
                  <span className="w-8 shrink-0 text-[11px] font-semibold uppercase text-zinc-400 dark:text-zinc-600">
                    {combine === "and" ? t("common.andUpper") : t("common.orUpper")}
                  </span>
                )}
                <select
                  value={row.field}
                ...
                {opTakesValue(row.op) &&
                  (isBool ? (
                    ...
                    >
                      ...
                      <option value="false">{t("admin.evalPanel.no")}</option>
                    </select>
                  ) : def?.options ? (
                    <select
                ...
                <button
                  ...
                  aria-label={t("admin.evalPanel.removeFilter")}
      ...
      <label className="mt-4 flex items-start gap-2 text-xs text-zinc-700 dark:text-zinc-300">
        ...
        />
        <span>
          {t("admin.evalPanel.iUnderstandThatThisRunsArbitrary")}
        </span>
      </label>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        >
          {running ? t("admin.evalPanel.sending") : t("admin.evalPanel.runOnTheClients")}
        </button>
        {result && (
          <span className="text-sm text-zinc-600 dark:text-zinc-400">
            {t("admin.evalPanel.sentTo")} <strong>{result.matched}</strong> {t("common.ofWord")} {result.total}{" "}
            {tc("admin.evalPanel.connectedClient", result.total)}.
          </span>
        )}
