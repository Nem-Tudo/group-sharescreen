  ...
  const activeStreams = useMemo<StreamEntry[]>(() => {
    ...
    for (const vs of state.videoSources ?? []) {
      list.push({
        ...
        badge: vs.kind,
        streamPath: `${origin}/stream/${encodeURIComponent(handle)}/video-source:${encodeURIComponent(vs.id)}`,
      });
    }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="rounded-full bg-purple-500/20 px-2 py-0.5 text-[10px] font-semibold text-purple-300 uppercase">
                            {entry.badge}
                          </span>
                        </div>
