...
// 1. Every action must exist in DEFAULT_SHORTCUTS and be empty strings by default
...
// 2. Every action must have definitions with id, label, description, and category
...
for (const def of SHORTCUT_DEFINITIONS) {
  ...
  if (def.id === "pushToTalk") {
    ...
    assert.equal(def.appOnly, true, "Push to talk must be app-only");
    assert.equal(def.held, true, "Push to talk acts on the key being held");
  } else if (def.category === "audio" || def.category === "clips") {
